...
| `GET` | `/checkout/options` | Listar direcciones, metodos, tipos y ordenes | Si |
| `POST` | `/checkout/addresses` | Registrar una direccion propia | Si |
| `POST` | `/checkout/payment-methods` | Registrar un metodo de pago propio | Si |
...
`GET /checkout/options` devuelve toda la informacion necesaria para construir
los selectores de pago:
...
Registrar una direccion con `POST /checkout/addresses`:
...
Registrar un metodo con `POST /checkout/payment-methods`:
...
La API nunca devuelve CVV ni numeros completos. Usa los IDs recibidos en
`/checkout/options`; no inventes valores locales.
...
Cada nueva busqueda reemplaza automaticamente los resultados anteriores. Al consultar el carrito, el resumen recibido en `data` reemplaza visualmente la busqueda activa, por lo que no se necesita un boton para ocultar resultados. Agregar un producto no descuenta inventario: el stock se valida y se descuenta al procesar el pago. Tras un pago exitoso, el frontend limpia el carrito y actualiza inmediatamente el stock de los resultados visibles usando las cantidades de `data.details`; no es necesario recargar la pagina.
...
| `orderId` | `data.order.orderId` del pago o `GET /checkout/options` |
| `addressId` | Selector cargado desde `GET /checkout/options` |
| `paymentMethodId` | Selector cargado desde `GET /checkout/options` |
...
- Los metodos de pago solo exponen tipo, titular, vencimiento y ultimos cuatro digitos; nunca CVV ni numero completo.
